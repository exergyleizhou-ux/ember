"""
集成测试: 现有检测器(经注册表)对着可开关漏洞的靶机端到端验证。
...
# ── AUTH-BYPASS ──
def test_auth_bypass_detected(scanner_mod, detectors_mod, make_vuln_server):
    server = make_vuln_server(vulns={"auth_bypass"})
    scanner = scanner_mod.Scanner(server.base_url)
    scanner.jwt = [{"path": "/jwt/thing", "method": "GET"}]
    detectors_mod.get("auth-bypass").run(scanner)
    found = _findings(scanner, "auth-bypass")
    assert found, "JWT 端点无 token 放行,未被检出 → 漏报"
    assert found[0]["severity"] == "high"


def test_auth_bypass_no_false_positive(scanner_mod, detectors_mod, make_vuln_server):
...
# ── OPS-ESCALATE ──
def test_ops_escalation_detected(scanner_mod, detectors_mod, make_vuln_server):
    server = make_vuln_server(vulns={"ops_escalation"})
    scanner = scanner_mod.Scanner(server.base_url)
    scanner.admin = [{"path": "/admin/thing", "method": "POST"}]
    detectors_mod.get("ops-escalate").run(scanner)
    found = _findings(scanner, "ops-escalate")
    assert found, "admin 端点对 buyer 放行,未被检出 → 漏报"
    assert found[0]["severity"] == "critical"


def test_ops_escalation_no_false_positive(scanner_mod, detectors_mod, make_vuln_server):
...
# ── IDOR ──
def test_idor_detected(scanner_mod, detectors_mod, make_vuln_server):
    server = make_vuln_server(vulns={"idor"})
    scanner = scanner_mod.Scanner(server.base_url)
    detectors_mod.get("idor").run(scanner)
    assert _findings(scanner, "idor"), "跨用户访问被放行,未被检出 → 漏报"


def test_idor_no_false_positive(scanner_mod, detectors_mod, make_vuln_server):
